...
import '../Model/picked_orders_model.dart';

class PickedOrderListItem extends StatelessWidget {
   PickedOrderListItem({super.key,this.order});

   OrderController                 vc = Get.find<OrderController>();
   PickedOrderDatum ?order;

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () async {
        var result = await Get.to(() => OrderDetailsView(orderId: order!.orderId,orderStatus: "Picked by courier",));
        if (result == 1) {}
      },
      child: Container(
